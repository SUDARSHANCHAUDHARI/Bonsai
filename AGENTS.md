...
Compact always-on ruleset for agents without skill support. This block is the
canonical copy; the adapter files (`.cursor/`, `.windsurf/`, `.clinerules/`,
`.github/copilot-instructions.md`, `.kiro/`) carry it verbatim. Keep them in
sync with `node scripts/check-rule-copies.js`.
...
- **lite** — build what's asked, name the smaller alternative in one line.
- **full** — the cut order enforced (default).
- **ultra** — YAGNI extremist: ship the one-liner and challenge the requirement.
